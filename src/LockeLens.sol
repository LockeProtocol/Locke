...
contract LockeLens {
     ...
     *
     */
    function currDepositTokensNotYetStreamed(IStream stream, address who)
        external
        view
        returns (uint256)
    {
        unchecked {
            ...
            if (acctTimeDelta > 0) {
                uint256 streamAmt =
                    uint256(acctTimeDelta) * tokens / (endStream - lastUpdate);
     ...
     *
     */
    function rewardsRemainingToAllocate(IStream stream)
        external
        view
        returns (uint256)
    {
    ...
    function currUnstreamed(IStream stream) external view returns (uint256) {
        ...
        return unstreamed
            - (uint256(tdelta) * unstreamed / (endStream - lastUpdate));
    }
}
